...
class Blinky:
    def __init__(self, mapa, tamanio_celda, velocidad=1, intervalo_movimiento=1):
        ...
        self.direccion_actual = (0, 1)  # Inicialmente Blinky se moverá hacia abajo.
        self.frame_actual = 0
        ...
        # Posición inicial de Blinky
        centro_x = mapa.num_columnas // 2
        centro_y = mapa.num_filas // 2
        self.posicion_inicial = (centro_x - 1, centro_y)  # Blinky inicia en la posición central, ligeramente desplazado
        self.posicion = self.posicion_inicial

        # Cargar imágenes de Blinky
        self.imagenes_base = [
            pygame.transform.scale(
                pygame.image.load(ruta).convert_alpha(),
                (self.tamanio_celda, self.tamanio_celda)
            ) for ruta in RUTA_IMAGEN_BLINKY
        ]
