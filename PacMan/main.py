import pygame
import json  # Para guardar y cargar la partida
import time
...
class JuegoPacman:
    def __init__(self):
        ...
        self.fuente_pequenia = pygame.font.Font(None, 33)
        ...
        estado = {
            ...
            "pinky": {
                ...
                "tiempo_restante_frightened": self.pinky.duracion_frightened - (
                            pygame.time.get_ticks() - self.pinky.inicio_frightened) if self.pinky.estado_frightened else 0
            },
            "clyde": {
                ...
                "tiempo_restante_frightened": self.clyde.duracion_frightened - (
                            pygame.time.get_ticks() - self.clyde.inicio_frightened) if self.clyde.estado_frightened else 0
            },
    ...
    def cargar_juego(self):
        try:
            ...
            # Pinky
            self.pinky.posicion = tuple(estado["pinky"]["posicion"])
            if estado["pinky"]["frightened"]:
                self.pinky.activar_frightened(estado["pinky"]["tiempo_restante_frightened"])
            else:
                self.pinky.desactivar_frightened()

            # Clyde
            self.clyde.posicion = tuple(estado["clyde"]["posicion"])
            if estado["clyde"]["frightened"]:
                self.clyde.activar_frightened(estado["clyde"]["tiempo_restante_frightened"])
            else:
                self.clyde.desactivar_frightened()
